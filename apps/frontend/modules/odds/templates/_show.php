...
<div class="sf_admin_form">
    ...
    <?php foreach ($configuration->getFormFields($form, 'show') as $fieldset => $fields): ?>
      <?php //include_partial('odds/form_fieldset', array('odds' => $odds, 'form' => $form, 'fields' => $fields, 'fieldset' => $fieldset)) ?>
		...
		<div id="sf_fieldset_<?php echo preg_replace('/[^a-z0-9_]/', '_', strtolower($fieldset)) ?>">
			...
			<?php if ($field->isPartial()): ?>
		      <?php include_partial('odds/'.$name, array('odds' => $odds, 'form' => $form, 'attributes' => $attributes instanceof sfOutputEscaper ? $attributes->getRawValue() : $attributes)) ?>
		    <?php elseif ($field->isComponent()): ?>
		      <?php include_component('odds', $name, array('odds' => $odds, 'form' => $form, 'attributes' => $attributes instanceof sfOutputEscaper ? $attributes->getRawValue() : $attributes)) ?>
		    <?php else: ?>
